(function (global, factory) {
	...
	var Collection = (function () {
		function Collection() {
			...
			if (this.class && !this.class.deserialize && !this.serializer) {
				throw new Error(
					"opt.class must have deserialize() method or opt.serializer must be defined"
				);
			}
			...
			this.askPeers =
				typeof opt.askPeers === "undefined" ? true : opt.askPeers;
		}
		...
		Collection.prototype._addToIndexes = async function _addToIndexes(
			serializedObject,
			node
		) {
			var _this = this;
		...
		Collection.prototype.get = function get() {
			...
			var matcher = function matcher(data, id, node) {
				...
				if (_this2.serializer) {
					opt.callback(
						_this2.serializer.deserialize(data, {
							id: id,
							gun: node.$,
						})
					);
					...
					opt.callback(
						_this2.class.deserialize(data, { id: id, gun: node.$ })
					);
				} else {
					opt.callback(data);
				}
	...
	var core_getIteratorMethod = (_core.getIteratorMethod = function (it) {
		if (it != undefined)
			return (
				it[ITERATOR$2] || it["@@iterator"] || _iterators[_classof(it)]
			);
	});
	...
	_export(_export.P + _export.R, "Promise", {
			...
			return this.then(
				isFunction
					? function (x) {
							return _promiseResolve(C, onFinally()).then(
								function () {
									return x;
								}
							);
		...
		_export(
			...
			"JSON",
			{
				stringify: function stringify(it) {
					...
					if (
						(!_isObject(replacer) && it === undefined) ||
						isSymbol(it)
					)
	...
	var possibleConstructorReturn = createCommonjsModule(function (
		module,
		exports
	) {
		exports.__esModule = true;
			...
			("__proto__" in {} // eslint-disable-line
				? (function (test, buggy, set) {
						try {
							set = _ctx(
								Function.call,
								_objectGopd.f(Object.prototype, "__proto__")
									.set,
								2
							);
							set(test, []);
	...
	try {
		isNode$2 =
			Object.prototype.toString.call(global$1.process) ===
			"[object process]";
	} catch (e) {}
		...
		return chain.then().then(function (layer) {
			...
			if (
				...
				(typeof layer === "undefined"
					? "undefined"
					: _typeof(layer)) !== "object" ||
				layer.constructor === Array
			) {
				return layer;
			}
			...
			var promises = _Object$keys(layer).map(function (key) {
				...
				if (
					!Gun.val.link.is(layer[key]) ||
					++bcount >= opts.maxBreadth
				) {
					return;
				}
				...
				return (opts.cache[key] = loadGunDepth(
					chain.get(key),
					maxDepth - 1,
					opts
				).then(function (data) {
	...
	var util = {
		...
		getHash: async function getHash(str) {
			...
			var hash = await Gun.SEA.work(str, undefined, undefined, {
				name: "SHA-256",
			});
			if (hash.length > 44) {
				throw new Error(
					"Gun.SEA.work returned an invalid SHA-256 hash longer than 44 chars: " +
						hash +
						". This is probably due to a sea.js bug on Safari."
				);
			}
			if (format === "hex") {
		...
		getDaySeparatorText: function getDaySeparatorText(
			date,
			dateStr,
			now,
			nowStr
		...
		(function () {
			...
			var PNGlib = function (width, height, depth) {
				...
				write(
					this.buffer,
					this.plte_offs,
					byte4(this.plte_size - 12),
					"PLTE"
				);
				write(
					this.buffer,
					this.trns_offs,
					byte4(this.trns_size - 12),
					"tRNS"
				);
				write(
					this.buffer,
					this.idat_offs,
					byte4(this.idat_size - 12),
					"IDAT"
				);
				write(
					this.buffer,
					this.iend_offs,
					byte4(this.iend_size - 12),
					"IEND"
				);
				...
				this.color = function (red, green, blue, alpha) {
					...
					if (typeof this.palette[color] == "undefined") {
						...
						this.palette[color] = String.fromCharCode(
							this.pindex++
						);
					}
				...
				this.getDump = function () {
					...
					function crc32(png, offs, size) {
						...
						for (var i = 4; i < size - 4; i += 1) {
							crc =
								_crc32[
									(crc ^ png[offs + i].charCodeAt(0)) & 0xff
								] ^
								((crc >> 8) & 0x00ffffff);
						}
	...
	var identicon = createCommonjsModule(function (module) {
		...
		(function () {
			...
			var Identicon = function (hash, options) {
				...
				this.options =
					typeof options === "object" ? options : this.defaults;
				...
				var saturation =
					this.options.saturation || this.defaults.saturation;
				var brightness =
					this.options.brightness || this.defaults.brightness;
				this.foreground =
					this.options.foreground ||
					this.hsl2rgb(hue, saturation, brightness);
			};
			...
			Svg.prototype = {
				...
				getBase64: function () {
					if ("function" === typeof btoa) {
						...
						return new Buffer(this.getDump(), "binary").toString(
							"base64"
						);
					} else {
	...
	var Attribute = (function () {
		...
		function Attribute(a, b) {
			...
			if (typeof a === "object") {
				if (typeof a.value !== "string") {
					throw new Error(
						"param1.value must be a string, got " +
							_typeof(a.value) +
							": " +
							_JSON$stringify(a.value)
					);
				}
				if (typeof a.type !== "string") {
					throw new Error(
						"param1.type must be a string, got " +
							_typeof(a.type) +
							": " +
							_JSON$stringify(a.type)
					);
				}
				b = a.value;
				a = a.type;
			...
			if (typeof a !== "string") {
				throw new Error(
					"First param must be a string, got " +
			...
			if (b) {
				if (typeof b !== "string") {
					throw new Error(
						"Second parameter must be a string, got " +
							(typeof b === "undefined"
								? "undefined"
								: _typeof(b)) +
							": " +
				...
				if (t) {
					...
					throw new Error(
						"Type of attribute was omitted and could not be guessed"
					);
				}
			}
		}
		...
		Attribute.prototype.identiconXml = function identiconXml() {
			...
			return util
				...
				.then(function (hash) {
					var identicon$$1 = new identicon(hash, {
						width: options.width,
						format: "svg",
					});
		...
		Attribute.prototype.identiconSrc = function identiconSrc() {
			...
			return util
				...
				.then(function (hash) {
					...
					return (
						"data:image/svg+xml;base64," + identicon$$1.toString()
					);
				});
		};
		...
		Attribute.prototype.identicon = function identicon$$1() {
			...
			if (options.showType) {
				...
				name.style.fontSize =
					options.width > 50 ? options.width / 4 + "px" : "10px";
	...
	var Key = (function () {
		...
		Key.getDefault = async function getDefault() {
			...
			if (util.isNode) {
				...
				if (!myKey) {
					throw new Error(
						"loading default key failed - check " +
							datadir +
							"/" +
							keyfile
					);
				...
				if (!myKey) {
					throw new Error(
						"loading default key failed - check localStorage iris.myKey"
					);
				}
			}
			return myKey;
	...
	var SignedMessage = (function () {
			...
			if (obj.sig) {
				if (typeof obj.sig !== "string") {
					throw new ValidationError(
						"SignedMessage signature must be a string"
					);
				}
				this.sig = obj.sig;
				this.getHash();
		...
		SignedMessage._getIterable = function _getIterable(authorOrRecipient) {
			...
			return (
				(_ref = {}),
				(_ref[_Symbol$iterator] = /*#__PURE__*/ regenerator.mark(
					function _callee() {
		...
		SignedMessage.prototype.getRecipientArray = function getRecipientArray() {
			return this.signedData.recipient
				? SignedMessage._getArray(this.signedData.recipient)
				: [];
		};
		...
		SignedMessage.prototype._validate = function _validate() {
			...
			if (typeof this.signedData !== "object") {
				throw new ValidationError(
					errorMsg + " signedData must be an object"
				);
			}
			...
			if (!d.type) {
				throw new ValidationError(
					errorMsg + " Missing type definition"
				);
			}
			if (!d.author) {
			...
			if (Array.isArray(d.author)) {
				throw new ValidationError(
					errorMsg + " Author must not be an array"
				);
			}
			...
			for (var attr in d.author) {
					...
					if (Array.isArray(d.author[attr])) {
						for (var i = 0; i < d.author[attr].length; i++) {
							if (typeof d.author[attr][i] !== "string") {
								throw new ValidationError(
									errorMsg +
										" Author attribute must be string, got " +
										attr +
										": [" +
										d.author[attr][i] +
										"]"
						...
						throw new ValidationError(
							errorMsg +
								" Author attribute must be string or array, got " +
								attr +
								": " +
								d.author[attr]
						);
				...
				if (attr === "keyID") {
					if (t !== "string") {
						throw new ValidationError(
							errorMsg + " Author keyID must be string, got " + t
						);
					}
					if (
						this.signerKeyHash &&
						d.author[attr] !== this.signerKeyHash
					) {
						throw new ValidationError(
							errorMsg +
								" If message has a keyID author, it must be signed by the same key"
						);
					}
				}
			}
			if (d.recipient) {
				if (typeof d.recipient !== "object") {
					throw new ValidationError(
						errorMsg + " Recipient must be object"
					);
				}
				if (Array.isArray(d.recipient)) {
					throw new ValidationError(
						errorMsg + " Recipient must not be an array"
					);
				}
				...
				for (var _attr in d.recipient) {
					...
					if (_t !== "string") {
						if (Array.isArray(d.recipient[_attr])) {
							for (
								var _i = 0;
								_i < d.recipient[_attr].length;
								_i++
							) {
								if (
									typeof d.recipient[_attr][_i] !== "string"
								) {
									throw new ValidationError(
										errorMsg +
											" Recipient attribute must be string, got " +
											_attr +
											": [" +
											d.recipient[_attr][_i] +
											"]"
									);
								}
							...
							throw new ValidationError(
								errorMsg +
									" Recipient attribute must be string or array, got " +
									_attr +
									": " +
									d.recipient[_attr]
							);
			...
			if (d.type === "rating") {
				...
				if (d.rating > d.maxRating) {
					throw new ValidationError(
						errorMsg + " Rating is above maxRating"
					);
				}
				if (d.rating < d.minRating) {
					throw new ValidationError(
						errorMsg + " Rating is below minRating"
					);
				}
				if (typeof d.context !== "string" || !d.context.length) {
					throw new ValidationError(
						errorMsg + " Rating messages must have a context field"
					);
				}
			}
			...
			if (d.type === "verification" || d.type === "unverification") {
				if (d.recipient.length < 2) {
					throw new ValidationError(
						errorMsg +
							" At least 2 recipient attributes are needed for a connection / disconnection. Got: " +
							d.recipient
					);
				}
			}
		...
		SignedMessage.prototype.isPositive = function isPositive() {
			return (
				this.signedData.type === "rating" &&
				this.signedData.rating >
					(this.signedData.maxRating + this.signedData.minRating) / 2
			);
		};
		...
		SignedMessage.createVerification = function createVerification(
			signedData,
			signingKey
		) {
		...
		SignedMessage.createRating = function createRating(
			signedData,
			signingKey
		) {
		...
		SignedMessage.prototype.getAuthor = function getAuthor(index) {
			for (
				var _iterator = this.getAuthorIterable(),
		...
		SignedMessage.prototype.getRecipient = function getRecipient(index) {
			...
			for (
				var _iterator2 = this.getRecipientIterable(),
		...
		SignedMessage.prototype.verify = async function verify() {
			if (!this.pubKey) {
				throw new ValidationError(
					errorMsg + " SignedMessage has no .pubKey"
				);
			}
			if (!this.sig) {
				throw new ValidationError(
					errorMsg + " SignedMessage has no .sig"
				);
			}
		...
		SignedMessage.setReaction = async function setReaction(
			gun,
			msg,
			reaction
		) {
			...
			gun.get("reactions").get(hash).put(reaction);
			gun.get("messagesByHash")
				.get(hash)
				.get("reactions")
				.get(this.rootContact.value)
				.put(reaction);
			gun.get("messagesByHash")
				.get(hash)
				.get("reactions")
	...
	var Contact = (function () {
		...
		Contact.create = function create(gun, data, index) {
			if (!data.linkTo && !data.attrs) {
				throw new Error(
					"You must specify either data.linkTo or data.attrs"
				);
			}
			if (data.linkTo && !data.attrs) {
		...
		Contact.getMostVerifiedAttributes = function getMostVerifiedAttributes(
			attrs
		) {
			...
			_Object$keys(attrs).forEach(function (k) {
				var a = attrs[k];
				var keyExists =
					_Object$keys(mostVerifiedAttributes).indexOf(a.type) > -1;
				a.verifications = isNaN(a.verifications) ? 1 : a.verifications;
				a.unverifications = isNaN(a.unverifications)
					? 0
					: a.unverifications;
				if (
					a.verifications * 2 > a.unverifications * 3 &&
					(!keyExists ||
						a.verifications - a.unverifications >
							mostVerifiedAttributes[a.type].verificationScore)
				) {
		...
		Contact.prototype.verified = async function verified(attribute) {
			...
			var mva = Contact.getMostVerifiedAttributes(attrs);
			return Object.prototype.hasOwnProperty.call(mva, attribute)
				? mva[attribute].attribute.value
				: undefined;
		};
		...
		Contact.prototype.profileCard = function profileCard(ipfs) {
			...
			this.gun.on(async function (data) {
				...
				linkEl.innerHTML =
					...
					link +
					'">' +
					((mva.type && mva.type.attribute.value) ||
						(mva.nickname && mva.nickname.attribute.value) ||
						linkTo.type + ":" + linkTo.value) +
					"</a><br>";
				linkEl.innerHTML +=
					'<small>Received: <span class="iris-pos">+' +
					(data.receivedPositive || 0) +
					'</span> / <span class="iris-neg">-' +
		...
		Contact.appendSearchWidget = function appendSearchWidget(
			parentElement,
			index
		) {
		...
		Contact.prototype.identicon = function identicon$$1() {
			...
			function setPie(data) {
				...
				if (options.outerGlow) {
					...
					if (data.receivedPositive > data.receivedNegative * 20) {
						boxShadow =
							"0px 0px " +
							(options.border * data.receivedPositive) / 50 +
							"px 0px #82FF84";
					} else if (
						data.receivedPositive <
						data.receivedNegative * 3
					) {
						boxShadow =
							"0px 0px " +
							(options.border * data.receivedNegative) / 10 +
							"px 0px #BF0400";
					}
						...
						bgImage =
							...
							(data.receivedPositive /
								(data.receivedPositive +
									data.receivedNegative)) *
								360 +
							"deg, transparent 50%, #3C763D 50%), linear-gradient(0deg, #3C763D 50%, transparent 50%)";
					} else {
						transform =
							"rotate(" +
							(((-data.receivedNegative /
						...
						bgImage =
							...
							(data.receivedNegative /
								(data.receivedPositive +
									data.receivedNegative)) *
								360 +
							"deg, transparent 50%, #A94442 50%), linear-gradient(0deg, #A94442 50%, transparent 50%)";
					}
				}
				...
				pie.style.opacity =
					((data.receivedPositive + data.receivedNegative) / 10) *
						0.5 +
					0.35;

				if (options.showDistance) {
					distance.textContent =
						typeof data.trustDistance === "number"
							? Contact._ordinal(data.trustDistance)
							: "\u2715";
				}
			}
			...
			function setIdenticonImg(data) {
				util.getHash(
					...
					var identiconImg = new identicon(hash, {
						width: options.width,
						format: "svg",
					});
					img.src =
						img.src ||
						"data:image/svg+xml;base64," + identiconImg.toString();
				});
			}
			...
			if (options.ipfs) {
				Contact.getAttrs(this.gun).then(function (attrs) {
					...
					if (mva.profilePhoto) {
						var go = function go() {
							options.ipfs
								.cat(mva.profilePhoto.attribute.value)
								.then(function (file) {
									var f = options.ipfs.types.Buffer.from(
										file
									).toString("base64");
									img.src = "data:image;base64," + f;
								});
						};
						options.ipfs.isOnline()
							? go()
							: options.ipfs.on("ready", go);
					}
				});
			}
	...
	var _parseInt =
		$parseInt(_stringWs + "08") !== 8 ||
		$parseInt(_stringWs + "0x16") !== 22
	...
	var Channel = (function () {
		function Channel(options) {
						...
						if (pub !== this.key.pub) {
							...
							if (sharedSecret && linkId) {
								...
								this.gun
									...
									.on(async function (encrypted) {
										var sharedKey = await Gun.SEA.decrypt(
											encrypted,
											sharedSecret
										);
										var encryptedChatRequest = await Gun.SEA.encrypt(
											_this.key.pub,
											sharedSecret
										); // TODO encrypt is not deterministic, it uses salt
										var channelRequestId = await util.getHash(
											encryptedChatRequest
										);
										util.gunAsAnotherUser(
											_this.gun,
											sharedKey,
											function (user) {
												user.get("chatRequests")
													.get(
														channelRequestId.slice(
															0,
															12
														)
													)
													.put(encryptedChatRequest);
											}
										);
									});
							}
					...
					if (k !== _this.key.pub) {
						_this.addParticipant(
							...
							_Object$assign(
								{},
								_this.DEFAULT_PERMISSIONS,
								options.participants[k]
							)
						);
					}
				});
				options.participants[this.key.pub] =
					options.participants[this.key.pub] ||
					_Object$assign({}, this.DEFAULT_PERMISSIONS);
				if (options.uuid) {
					// It's a group channel
			...
			this.onTheir("participants", function (participants, k, from) {
				...
				for (var i = 0; i < keys.length; i++) {
					if (
						_this.participants[keys[i]].admin ||
						_this.participants[keys[i]].inviter
					) {
						hasAdmin = true;
						break;
					}
				...
				if (
					_this.participants[from] &&
					(_this.participants[from].admin ||
						_this.participants[from].inviter)
				) {
					if (typeof participants === "object") {
						var before = _JSON$stringify(_this.participants);
						_this.participants = _Object$assign(
							_this.participants,
							participants
						);
		...
		Channel.prototype.getTheirSecretUuid = function getTheirSecretUuid(
			pub
		) {
			var _this2 = this;
		...
		Channel.prototype.getTheirGroupSecret = function getTheirGroupSecret(
			pub
		) {
			var _this3 = this;
		...
		Channel.prototype.mute = async function mute(participant) {
			this.gun
				.user(participant)
				.get(this.theirSecretUuids[participant])
				.off();
			// TODO: persist
		};
		...
		Channel.prototype.getMySecretUuid = async function getMySecretUuid() {
			if (!this.mySecretUuid) {
				...
				this.mySecretUuid = await util.getHash(
					mySecretHash + this.uuid
				);
			}
		...
		Channel.prototype.getSecret = async function getSecret(pub) {
			if (!this.secrets[pub]) {
				var epub = await util.gunOnceDefined(
					this.gun.user(pub).get("epub")
				);
		...
		Channel.getOurSecretChannelId = async function getOurSecretChannelId(
			gun,
			pub,
			pair
		) {
		...
		Channel.getTheirSecretChannelId = async function getTheirSecretChannelId(
			gun,
			pub,
			pair
		) {
		...
		Channel.getChannels = async function getChannels(
			gun,
			keypair,
			callback
		) {
			var listenToChatLinks =
			...
			gun.user()
				.get("chats")
				.map()
				.on(async function (value, ourSecretChannelId) {
					if (value && !seen[ourSecretChannelId]) {
						seen[ourSecretChannelId] = true;
						if (ourSecretChannelId.length > 44) {
							gun.user()
								.get("chats")
								.get(ourSecretChannelId)
								.put(null);
							return;
						}
						var encryptedChatId = await util.gunOnceDefined(
							gun
								.user()
								.get("chats")
								.get(ourSecretChannelId)
						...
						var chatId = await Gun.SEA.decrypt(
							encryptedChatId,
							mySecret
						);
		...
		Channel.prototype.getOurSecretChannelId = async function getOurSecretChannelId(
			pub
		) {
			if (!this.ourSecretChannelIds[pub]) {
				var secret = await this.getSecret(pub);
				this.ourSecretChannelIds[pub] = await util.getHash(
					secret + pub
				);
			}
		...
		Channel.prototype.getTheirSecretChannelId = async function getTheirSecretChannelId(
			pub
		) {
			if (!this.theirSecretChannelIds[pub]) {
				var secret = await this.getSecret(pub);
				this.theirSecretChannelIds[pub] = await util.getHash(
					secret + this.key.pub
				);
			}
		...
		Channel.prototype.getMessages = async function getMessages(callback) {
			...
			this.getCurrentParticipants().forEach(async function (pub) {
				if (pub !== _this4.key.pub) {
					...
					if (_this4.uuid) {
						theirSecretChannelId = await _this4.getTheirSecretUuid(
							pub
						);
					} else {
						theirSecretChannelId = await _this4.getTheirSecretChannelId(
							pub
						);
					}
					_this4.gun
						.user(pub)
						.get("chats")
						.get(theirSecretChannelId)
						.get("msgs")
						.map()
						.once(function (data, key) {
							_this4.messageReceived(
								callback,
								data,
								_this4.uuid || pub,
								false,
				...
				if (!_this4.uuid) {
					...
					var ourSecretChannelId = await _this4.getOurSecretChannelId(
						pub
					);
					_this4.user
						.get("chats")
		...
		Channel.prototype.messageReceived = async function messageReceived(
			callback,
			data,
			channelId,
			selfAuthored,
			...
			var secret = this.uuid
				? await this.getTheirGroupSecret(from)
				: await this.getSecret(channelId);
		...
		Channel.prototype.getLatestMsg = async function getLatestMsg(callback) {
			...
			var callbackIfLatest = async function callbackIfLatest(msg, info) {
				if (!_this5.latest) {
					...
					var t =
						typeof _this5.latest.time === "string"
							? _this5.latest.time
							: _this5.latest.time.toISOString();
					if (t < msg.time) {
						_this5.latest = msg;
						callback(msg, info);
					}
		...
		Channel.prototype.setMyMsgsLastSeenTime = async function setMyMsgsLastSeenTime(
			time
		) {
		...
		Channel.prototype.getMyMsgsLastSeenTime = async function getMyMsgsLastSeenTime(
			callback
		) {
			var _this6 = this;
		...
		Channel.prototype.getTheirMsgsLastSeenTime = async function getTheirMsgsLastSeenTime(
			callback
		) {
			var _this7 = this;
		...
		Channel.prototype.addParticipant = async function addParticipant(pub) {
			...
			if (
				this.secrets[pub] &&
				_JSON$stringify(this.secrets[pub]) ===
					_JSON$stringify(permissions)
			) {
			...
			if (save) {
				...
				this.gun
					...
					.get(ourSecretChannelId)
					.get("pub")
					.put(await Gun.SEA.encrypt({ pub: pub }, mySecret));
			}
			if (this.uuid) {
		...
		Channel.prototype.send = async function send(msg) {
			...
			if (this.uuid) {
				var encrypted = await Gun.SEA.encrypt(
					_JSON$stringify(msg),
				...
				for (var i = 0; i < keys.length; i++) {
					var _encrypted = await Gun.SEA.encrypt(
						_JSON$stringify(msg),
						await this.getSecret(keys[i])
					);
					var ourSecretChannelId = await this.getOurSecretChannelId(
						keys[i]
					);
					this.user
						.get("chats")
		...
		Channel.prototype.put = async function put(key, value) {
			return (this.uuid ? this.putGroup : this.putDirect).call(
				this,
				key,
				value
			);
		...
		Channel.prototype.putGroup = async function putGroup(key, value) {
			if (key === "msgs") {
				throw new Error(
					"Sorry, you can't overwrite the msgs field which is used for .send()"
				);
			}
			var encrypted = await Gun.SEA.encrypt(
				_JSON$stringify(value),
				this.getMyGroupSecret()
			);
			var mySecretUuid = await this.getMySecretUuid();
		...
		Channel.prototype.onDirect = async function onDirect(
			key,
			callback,
			from
		) {
		...
		Channel.prototype.onGroup = async function onGroup(
			key,
			callback,
			from
		) {
		...
		Channel.prototype.onMy = async function onMy(key, callback) {
			return (this.uuid ? this.onMyGroup : this.onMyDirect).call(
				this,
				key,
				callback
			);
		...
		Channel.prototype.onMyDirect = async function onMyDirect(
			key,
			callback
		) {
			var _this11 = this;
			...
			if (typeof callback !== "function") {
				throw new Error(
					"onMy callback must be a function, got " +
			...
			var _loop = async function _loop(i) {
				var ourSecretChannelId = await _this11.getOurSecretChannelId(
					keys[i]
				);
				_this11.gun
					.user()
					...
					.on(async function (data) {
						var decrypted = await Gun.SEA.decrypt(
							data,
							await _this11.getSecret(keys[i])
						);
						if (decrypted) {
							callback(
								typeof decrypted.v !== "undefined"
									? decrypted.v
									: decrypted,
								key
							);
		...
		Channel.prototype.onTheir = async function onTheir(
			key,
			callback,
			from
		) {
			return (this.uuid ? this.onTheirGroup : this.onTheirDirect).call(
				this,
				key,
				callback,
				from
		...
		Channel.prototype.onTheirDirect = async function onTheirDirect(
			key,
			callback,
			from
		) {
			...
			if (typeof callback !== "function") {
				throw new Error(
					"onTheir callback must be a function, got " +
						(typeof callback === "undefined"
							? "undefined"
							: _typeof(callback))
				);
			...
			keys.forEach(async function (pub) {
				...
				var theirSecretChannelId = await _this12.getTheirSecretChannelId(
					pub
				);
				_this12.gun
					.user(pub)
		...
		Channel.prototype.onTheirGroup = async function onTheirGroup(
			key,
			callback,
			from
		) {
			...
			keys.forEach(async function (pub) {
				...
				_this13.gun
					...
					.on(async function (data) {
						var decrypted = await Gun.SEA.decrypt(
							data,
							await _this13.getTheirGroupSecret(pub)
						);
						if (decrypted) {
							callback(
								typeof decrypted.v !== "undefined"
		...
		Channel.addChatButton = function addChatButton() {
			...
			if (!options.channelOptions) {
				throw new Error(
					"addChatButton missing options.channelOptions param"
				);
			}
			util.injectCss();
			var channel = void 0,
			...
			btn.innerHTML =
				'<svg style="margin-right:7px;margin-bottom: -0.2em" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 510 510" xml:space="preserve"><path fill="currentColor" d="M459,0H51C22.95,0,0,22.95,0,51v459l102-102h357c28.05,0,51-22.95,51-51V51C510,22.95,487.05,0,459,0z M102,178.5h306v51 H102V178.5z M306,306H102v-51h204V306z M408,153H102v-51h306V153z"></path></svg> ' +
				options.label;
		...
		Channel.prototype.getSimpleLink = function getSimpleLink() {
			...
			if (this.uuid) {
				...
				return (
					urlRoot + "?vibeWith=" + this.getCurrentParticipants()[0]
				);
			}
		};
		...
		Channel.prototype.getChatBox = function getChatBox() {
			...
			var minimize = util.createElement(
				"span",
				"iris-chat-minimize",
				header
			);
			...
			var headerText = util.createElement(
				"div",
				"iris-chat-header-text",
				header
			);
			var onlineIndicator = util.createElement(
				"span",
				"iris-online-indicator",
				headerText
			);
			...
			close.addEventListener("click", function () {
				...
				var openChatBtn = document.getElementById(
					"iris-chat-open-button"
				);
				if (openChatBtn) {
			...
			var messages = util.createElement(
				"div",
				"iris-chat-messages",
				chatBox
			);

			var typingIndicator = util.createElement(
				"div",
				"iris-typing-indicator",
				chatBox
			);
			...
			this.getTyping(function (isTyping) {
				typingIndicator.setAttribute(
					"class",
					"iris-typing-indicator" + (isTyping ? " yes" : "")
				);
			});

			var inputWrapper = util.createElement(
				"div",
				"iris-chat-input-wrapper",
				chatBox
			);
			var textArea = util.createElement(
				"textarea",
				undefined,
				inputWrapper
			);
			...
			if (util.isMobile) {
				var sendBtn = util.createElement(
					"button",
					undefined,
					inputWrapper
				);
			...
			if (participants.length) {
				...
				Channel.getOnline(this.gun, pub, function (status) {
					...
					var undelivered = messages.querySelectorAll(
						".iris-chat-message:not(.delivered)"
					);
					undelivered.forEach(function (msg) {
						if (
							msg.getAttribute("data-time") <= status.lastActive
						) {
			...
			this.getTheirMsgsLastSeenTime(function (time) {
				...
				unseen.forEach(function (indicator) {
					var msgEl =
						indicator.parentElement.parentElement.parentElement;
			...
			this.getMessages(function (msg, info) {
				...
				if (info.selfAuthored) {
					var cls =
						_this16.theirMsgsLastSeenTime >= msg.time
							? "iris-seen yes"
							: "iris-seen";
					var seenIndicator = util.createElement("span", cls, time);
					seenIndicator.innerHTML =
						' <svg version="1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 59 42"><polygon fill="currentColor" points="40.6,12.1 17,35.7 7.4,26.1 4.6,29 17,41.3 43.4,14.9"></polygon><polygon class="iris-delivered-checkmark" fill="currentColor" points="55.6,12.1 32,35.7 29.4,33.1 26.6,36 32,41.3 58.4,14.9"></polygon></svg>';
				}
				...
				var msgEl = util.createElement(
					"div",
					(info.selfAuthored ? "our" : "their") + " iris-chat-message"
				);
				...
				for (var i = messages.children.length; i >= 0; i--) {
					if (i === 0) {
						...
						var t = messages.children[i - 1].getAttribute(
							"data-time"
						);
						if (t && t < msg.time) {
							messages.children[i - 1].insertAdjacentElement(
								"afterend",
								msgEl
							);
							break;
		...
		Channel.setOnline = function setOnline(gun, isOnline) {
			if (isOnline) {
				...
				var update = function update() {
					gun.user()
						.get("lastActive")
						.put(new Date(Gun.state()).toISOString());
				};
				update();
				...
				.on(function (lastActive) {
					...
					if (lastActiveDate.getFullYear() === 1970) {
						...
						lastActiveDate = new Date(
							lastActiveDate.getTime() * 1000
						);
						lastActive = lastActiveDate.toISOString();
					}
					var isOnline =
						lastActiveDate > now - 10 * 1000 &&
						lastActive < now + 30 * 1000;
		...
		Channel.formatChatLink = function formatChatLink(
			urlRoot,
			pub,
			sharedSecret,
			linkId
			...
			return (
				...
				"&s=" +
				encodeURIComponent(sharedSecret) +
				"&k=" +
		...
		Channel.createChatLink = async function createChatLink(gun, key) {
			...
			var encryptedSharedKey = await Gun.SEA.encrypt(
				sharedKeyString,
				sharedSecret
			);
			var ownerSecret = await Gun.SEA.secret(key.epub, key);
			var ownerEncryptedSharedKey = await Gun.SEA.encrypt(
				sharedKeyString,
				ownerSecret
			);
		...
		Channel.getMyChatLinks = async function getMyChatLinks(gun, key) {
			...
			user.get("chatLinks")
				...
				.on(function (data, linkId) {
					...
					user.get("chatLinks")
						.get(linkId)
						.get("ownerEncryptedSharedKey")
						.on(async function (enc) {
							...
							var sharedSecret = await Gun.SEA.secret(
								sharedKey.epub,
								sharedKey
							);
							...
							if (subscribe) {
								gun.user(sharedKey.pub)
									.get("chatRequests")
									.map()
									.on(async function (encPub, requestId) {
										if (!encPub) {
											return;
										}
	...
	async function searchText(node, callback, query, limit) {
		...
		node.map().once(function (value, key) {
			...
			if (key.indexOf(query) === 0) {
				if (
					typeof limit === "number" &&
					_Object$keys(seen).length >= limit
				) {
					return;
				}
	...
	var SocialNetwork = (function () {
		function SocialNetwork(options) {
			...
			if (options.pubKey) {
				...
				this.rootContact = new Attribute({
					type: "keyID",
		...
		SocialNetwork.prototype._init = async function _init() {
			...
			this.gun
				.get("identitiesBySearchKey")
				.put(user.top("identitiesBySearchKey"));
			this.gun
				.get("identitiesByTrustDistance")
				.put(user.top("identitiesByTrustDistance"));
			this.gun.get("messages").put(user.top("messages"));
			this.gun
				.get("messagesByTimestamp")
				.put(user.top("messagesByTimestamp"));
			this.gun.get("messagesByHash").put(user.top("messagesByHash"));
			this.gun
				.get("messagesByDistance")
				.put(user.top("messagesByDistance"));

			this.messages = new Collection$2({
				gun: this.gun,
				class: SignedMessage,
				indexes: ["time", "trustDistance"],
			});
			...
			var id = Contact.create(
				g,
				{ trustDistance: 0, linkTo: this.rootContact, attrs: attrs },
				this
			);
			await this._addContactToIndexes(id.gun);
			if (this.options.self) {
				var recipient = _Object$assign(this.options.self, {
					keyID: this.rootContact.value,
				});
				SignedMessage.createVerification(
					{ recipient: recipient },
					keypair
				).then(function (msg) {
							...
							if (val) {
								...
								_this3.gun
									...
									.map(function (val, key) {
										var d = _Number$parseInt(
											key.split(":")[0]
										);
										if (
											!isNaN(d) &&
											d <=
												_this3.options.indexSync
													.subscribe.maxMsgDistance
										) {
											SignedMessage.fromSig(val).then(
												function (msg) {
													if (
														_this3.options.indexSync
															.msgTypes.all ||
		...
		SocialNetwork.prototype.debug = function debug() {
			var d =
				util.isNode && process$3.env.DEBUG
					? process$3.env.DEBUG === "true"
					: this.options.debug;
			if (d) {
		...
		SocialNetwork.getMsgIndexKey = function getMsgIndexKey(msg) {
			...
			var key =
				...
				":" +
				Math.floor(
					Date.parse(msg.signedData.time || msg.signedData.timestamp)
				) +
				":" +
				(msg.ipfs_hash || msg.hash).substr(0, 9);
			return key;
		};
		...
		SocialNetwork.getMsgIndexKeys = async function getMsgIndexKeys(msg) {
			...
			if (msg.signedData.type === "chat") {
				if (msg.signedData.recipient.uuid) {
					...
					keys.chatMessagesByUuid[msg.signedData.recipient.uuid] =
						msg.signedData.time + ":" + hashSlice;
				}
				return keys;
			}
			...
			keys.messagesByDistance = [
				distance + ":" + keys.messagesByTimestamp[0],
			];
			keys.messagesByType = [
				msg.signedData.type + ":" + timestamp + ":" + hashSlice,
			];
			...
			for (var i = 0; i < authors.length; i++) {
				if (authors[i].isUniqueType()) {
					keys.messagesByAuthor[authors[i].uri()] =
						msg.signedData.timestamp + ":" + hashSlice;
				}
			}
			...
			for (var _i = 0; _i < recipients.length; _i++) {
				if (recipients[_i].isUniqueType()) {
					keys.messagesByRecipient[recipients[_i].uri()] =
						msg.signedData.timestamp + ":" + hashSlice;
				}
			}

			if (
				["verification", "unverification"].indexOf(
					msg.signedData.type
				) > -1
			) {
		...
		SocialNetwork.prototype.getContactIndexKeys = async function getContactIndexKeys(
			contact,
			hash
		) {
			var indexKeys = {
				identitiesByTrustDistance: [],
				identitiesBySearchKey: [],
			};
			var d = void 0;
			...
			function addIndexKey(a) {
				...
				distance = _Number$isNaN(distance) ? 99 : distance;
				distance = ("00" + distance).substring(
				...
				if (key !== lowerCaseKey) {
					...
					indexKeys.identitiesByTrustDistance.push(
						distance + ":" + lowerCaseKey
					);
				}
				if (v.indexOf(" ") > -1) {
				...
				if (key.match(/^http(s)?:\/\/.+\/[a-zA-Z0-9_]+$/)) {
					var split = key.split("/");
					indexKeys.identitiesBySearchKey.push(
						split[split.length - 1]
					);
					indexKeys.identitiesByTrustDistance.push(
						distance + ":" + split[split.length - 1]
					);
				}
			}
		...
		SocialNetwork.prototype.addContact = async function addContact(
			attributes
		) {
			var follow =
				arguments.length > 1 && arguments[1] !== undefined
		...
		SocialNetwork.prototype.getContact = function getContact(a, b) {
			...
			if (!a) {
				throw new Error(
					"getContact failed: first param must be defined"
				);
			}
			var attr = a;
			...
			if (reload) {
				...
				var updateContactByLinkedAttribute = function updateContactByLinkedAttribute(
					attribute
				) {
					_this4.gun
						.get("verificationsByRecipient")
						.get(attribute.uri())
						.map()
						.once(function (val) {
							...
							for (var i = 0; i < recipients.length; i++) {
								...
								if (
									...
									_this4.gun
										.get("messagesByRecipient")
										.get(a2.uri())
										.map()
										.once(async function (val) {
											var m2 = SignedMessage.fromSig(val);
											var m2hash = await m2.getHash();
											if (
												!Object.prototype.hasOwnProperty.call(
													o.received.hasOwnProperty,
													m2hash
												)
											) {
				...
				return new Contact(
					this.gun.get("identitiesBySearchKey").get(attr.uri()),
					attr,
					this
				);
			}
					...
					.once(function (val, key) {
						if (val) {
							_this5.gun
								...
								.on(function (id, k) {
									if (
										_Object$keys(seen).length >= opt.limit
									) {
										// TODO: turn off .map cb
										return;
									}
		...
		SocialNetwork.prototype.getChannels = async function getChannels(
			callback
		) {
			var _this6 = this;
			...
			this.gun
				.get("trustedIndexes")
				.map()
				.on(async function (value, pub) {
					if (value && pub) {
						var theirSecretChannelId = await Channel.getTheirSecretChannelId(
							_this6.gun,
							pub,
							_this6.options.keypair
						);
						_this6.gun
							.user(pub)
							.get("chats")
		...
		SocialNetwork.prototype._getMsgs = async function _getMsgs(
			msgIndex,
			callback,
			limit,
			cursor,
		...
		SocialNetwork.prototype._addContactToIndexes = async function _addContactToIndexes(
			id
		) {
			...
			var indexKeys = await this.getContactIndexKeys(
				id,
				hash.substr(0, 6)
			);
		...
		SocialNetwork.prototype._getSentMsgs = async function _getSentMsgs(
			contact,
			options
		) {
			var _this7 = this;
		...
		SocialNetwork.prototype._getReceivedMsgs = async function _getReceivedMsgs(
			contact,
			options
		) {
			var _this8 = this;
		...
		SocialNetwork.prototype.getChatMsgs = async function getChatMsgs(
			uuid,
			options
		) {
			var _this9 = this;
			...
			this._getMsgs(
				this.gun.get("chatMessagesByUuid").get(uuid),
				options.callback,
				options.limit,
				options.cursor,
				true,
		...
		SocialNetwork.prototype._getAttributeTrustDistance = async function _getAttributeTrustDistance(
			a
		) {
		...
		SocialNetwork.prototype._getMsgTrustDistance = async function _getMsgTrustDistance(
			msg
		) {
			...
			for (
				var _iterator = msg.getAuthorArray(),
					_isArray = Array.isArray(_iterator),
					_i4 = 0,
					_iterator = _isArray ? _iterator : _getIterator(_iterator);
				;
		...
		SocialNetwork.prototype._updateMsgRecipientContact = async function _updateMsgRecipientContact(
			...
			var contactIndexKeysBefore = await this.getContactIndexKeys(
				recipient,
				hash.substr(0, 6)
			);
			...
			if (
				...
				var _loop = function _loop() {
					...
					_Object$keys(attrs).forEach(function (k) {
						...
						if (a.equals(attrs[k])) {
							attrs[k].verifications =
								(attrs[k].verifications || 0) + v.verifications;
							attrs[k].unverifications =
								(attrs[k].unverifications || 0) +
								v.unverifications;
							hasAttr = true;
						}
					});
					if (!hasAttr) {
						attrs[a.uri()] = _Object$assign(
				...
				for (
					var _iterator2 = msg.getRecipientArray(),
						_isArray2 = Array.isArray(_iterator2),
						_i5 = 0,
						_iterator2 = _isArray2
							? _iterator2
				...
				var k =
					(mva.keyID && mva.keyID.attribute.value) ||
					(mva.uuid && mva.uuid.attribute.value) ||
					hash;
				console.log("k", k);
			...
			if (msg.signedData.type === "rating") {
				...
				if (msg.isPositive()) {
					if (
						typeof id.trustDistance !== "number" ||
						msg.distance + 1 < id.trustDistance
					) {
			...
			var contactIndexKeysAfter = await this.getContactIndexKeys(
				recipient,
				hash.substr(0, 6)
			);
			...
			for (var i = 0; i < indexesBefore.length; i++) {
				...
				for (var j = 0; j < contactIndexKeysBefore[index].length; j++) {
					...
					if (
						!contactIndexKeysAfter[index] ||
						contactIndexKeysAfter[index].indexOf(key) === -1
					) {
		...
		SocialNetwork.prototype._updateMsgAuthorContact = async function _updateMsgAuthorContact(
			msg,
			msgIndexKey,
			author
		) {
		...
		SocialNetwork.prototype._updateContactProfilesByMsg = async function _updateContactProfilesByMsg(
			msg,
			authorIdentities,
			recipientIdentities
		) {
			...
			var ids = _Object$values(
				_Object$assign({}, authorIdentities, recipientIdentities)
			);
			for (var i = 0; i < ids.length; i++) {
				...
				if (
					...
					start = new Date();
					await this._updateMsgRecipientContact(
						msg,
						msgIndexKey,
						ids[i].gun
					);
				...
				if (
					...
					start = new Date();
					await this._updateMsgAuthorContact(
						msg,
						msgIndexKey,
						ids[i].gun
					);
		...
		SocialNetwork.prototype.removeTrustedIndex = async function removeTrustedIndex(
			gunUri
		) {
			this.gun.get("trustedIndexes").get(gunUri).put(null);
		};

		SocialNetwork.prototype.addTrustedIndex = async function addTrustedIndex(
			gunUri
		) {
			var _this10 = this;
			...
			if (this.options.indexSync.importOnAdd.enabled) {
				await util.timeoutPromise(
					new _Promise(function (resolve) {
						...
						var messages = new Collection$2({
							gun: gun,
							class: SignedMessage,
							indexes: ["trustDistance"],
						});
						messages.get({
							callback: callback,
							orderBy: "trustDistance",
							desc: false,
						});
					}),
					10000
		...
		SocialNetwork.prototype._updateContactIndexesByMsg = async function _updateContactIndexesByMsg(
			msg
		) {
			...
			start = new Date();
			for (
				var _iterator3 = msg.getAuthorArray(),
				...
				if (!isNaN(td)) {
					...
					if (
						scores &&
						scores.verifier &&
						msg.signedData.type === "verification"
					) {
			...
			start = new Date();
			for (
				var _iterator4 = msg.getRecipientArray(),
				...
				if (
					selfAuthored &&
					_a4.type === "keyID" &&
					_a4.value !== this.rootContact.value
				) {
			...
			if (!_Object$keys(recipientIdentities).length) {
				...
				var u = void 0;
				for (
					var _iterator5 = msg.getRecipientArray(),
				...
				var trustDistance =
					msg.isPositive() && typeof msg.distance === "number"
						? msg.distance + 1
						: false;
				var _start = new Date();
			...
			return this._updateContactProfilesByMsg(
				msg,
				authorIdentities,
				recipientIdentities
			);
		...
		SocialNetwork.prototype.addMessage = async function addMessage(msg) {
			...
			if (!this.writable) {
				throw new Error(
					"Cannot write to a read-only index (initialized with options.pubKey)"
				);
			}
			...
			if (msg.constructor.name !== "SignedMessage") {
				throw new Error(
					"addMessage failed: param must be a SignedMessage, received " +
			...
			var node = this.gun
				...
				.get("messagesByHash")
				.get(hash)
				.put(obj); // TODO: needs fix to https://github.com/amark/gun/issues/719
			start = new Date();
			var d = await this._getMsgTrustDistance(msg);
			msg.distance = Object.prototype.hasOwnProperty.call(msg, "distance")
				? msg.distance
				: d; // eslint-disable-line require-atomic-updates
			this.debug("----");
			...
			if (this.options.ipfs) {
				try {
					...
					console.error(
						"adding msg " + msg + " to ipfs failed: " + e
					);
				}
			}
			if (msg.signedData.type !== "chat") {
		...
		SocialNetwork.prototype.addMessages = async function addMessages(msgs) {
			...
			if (Array.isArray(msgs)) {
				this.debug(
					"sorting " + msgs.length + " messages onto a search tree..."
				);
				for (var i = 0; i < msgs.length; i++) {
					for (
						var _iterator6 = msgs[i].getAuthorArray(),
			...
			var _loop2 = async function _loop2() {
				...
				while (author && knownContact) {
					if (author.indexOf(knownContact.key) === 0) {
						try {
							await util.timeoutPromise(
								_this11.addMessage(msgsByAuthor[author], {
									checkIfExists: true,
								}),
								10000
							);
							...
							_this11.debug(
								"adding failed:",
								e,
								_JSON$stringify(msgsByAuthor[author], null, 2)
							);
						}
						msgAuthors.splice(i, 1);
						author =
							i < msgAuthors.length ? msgAuthors[i] : undefined;
		...
		SocialNetwork.prototype.getMessageByHash = function getMessageByHash(
			hash
		) {
			var _this12 = this;
			...
			return new _Promise(function (resolve) {
				var resolveIfHashMatches = async function resolveIfHashMatches(
					d,
					fromIpfs
				) {
					...
					if (h === hash || (isIpfsUri && !_this12.options.ipfs)) {
						...
						if (
							!fromIpfs &&
							_this12.options.ipfs &&
							_this12.writable &&
							!republished
						) {
							m.saveToIpfs(_this12.options.ipfs).then(function (
								ipfsUri
							) {
								obj.ipfsUri = ipfsUri;
								_this12.gun
				...
				if (isIpfsUri && _this12.options.ipfs) {
					_this12.options.ipfs.cat(hash).then(function (file) {
						var s = _this12.options.ipfs.types.Buffer.from(
							file
						).toString("utf8");
				...
				_this12.gun
					...
					.on(function (d) {
						_this12.debug(
							"got msg " + hash + " from own gun index"
						);
		...
		SocialNetwork.prototype.getMessagesByTimestamp = function getMessagesByTimestamp(
			callback,
			limit,
			cursor
		) {
			...
			var cb = async function cb(msg) {
				if (
					(!limit || _Object$keys(seen).length < limit) &&
					!Object.prototype.hasOwnProperty.call(seen, msg.hash)
				) {
					seen[msg.hash] = true;
					callback(msg);
				}
			...
			this._getMsgs(
				this.gun.get("messagesByTimestamp"),
				cb,
				limit,
				cursor,
				desc,
		...
		SocialNetwork.prototype.getMessagesByDistance = function getMessagesByDistance(
			callback,
			limit,
			cursor,
			desc,
			...
			this._getMsgs(
				this.gun.get("messagesByDistance"),
				cb,
				limit,
				cursor,
				desc,
