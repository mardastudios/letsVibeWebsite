...
function getRandomAvatar() {
  ...
  Object.keys(AVATAR).forEach((attrib) => {
    ...
    avatarData[attrib] = attribVals[parseInt(Math.random() * attribVals.length)];
  });
  return avatarData;
}
...
function init() {
    ...
    var option;
    var myAvatarData;
    gun.user().get('profile').get('avatar').on(av => {
        $("#avatar-edit").empty();
        delete av['_'];
        myAvatarData = av;
        Object.keys(AVATAR).forEach(k => {
            title = $('<p>');
            selector = $('<select class="custom-select mx-4" style="width: 100%;display: block;">');
