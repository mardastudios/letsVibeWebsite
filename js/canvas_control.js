 ...
 */
export default function CanvasControl(canvas, elements, callbackFunc) {
	this._canvas = canvas;
	...
	canvas.addEventListener(
		...
		function (event) {
			...
			if (
				currentEventTime - that._lastMoveEventTime >
				that._minimumThreshold
			) {
...
CanvasControl.prototype.getNearestElement = function (cursorPosition) {
	...
	for (let i = 0; i < this._elements.length; i++) {
		if (this._elements[i].clickable == true) {
			let dx =
				this._elements[i].x * this._canvas.width - cursorPosition.x;
			let dy =
				this._elements[i].y * this._canvas.height - cursorPosition.y;
...
CanvasControl.prototype._cursorUpdateFunc = function (cursorPosition) {
	if (this._selected.index > -1) {
		...
		this._elements[this._selected.index].y = Math.max(
			...
			Math.min(
				1,
				(cursorPosition.y + this._selected.yOffset) /
					this._canvas.height
			)
		);
		this.invokeCallback();
